...
const EditListing = () => {
    ...
    const [photos, setPhotos] = useState(listingData.photos || []);
    ...
    useEffect(() => {
        const fetchListingData = async () => {
            try {
                ...
                setType(data.type); // Set the type
                setFormLocation({
                ...
                setGuestCount(data.guestCount); // Set guest count
                setBedroomCount(data.bedroomCount); // Set bedroom count
                setBedCount(data.bedCount); // Set bed count
                setBathroomCount(data.bathroomCount); // Set bathroom count
                setAmenities(data.amenities); // Set amenities
                setPhotos(data.photos); // Set photos
                setFormDescription({
                    title: data.title,
                ...
                console.error("Fetch listing data failed:", err.message);
                setLoading(false);
            }
        };
    ...
    const handleEdit = async (e) => {
        ...
        try {
            ...
            const response = await fetch(`http://localhost:5000/properties/edit/${listingId}`, {
                method: "PUT",
                headers: {
            ...
            console.error("Edit listing failed", err.message);
        }
    };
